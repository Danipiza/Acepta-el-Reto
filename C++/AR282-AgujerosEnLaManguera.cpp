#include <iostream>
#include <iomanip> // biblioteca para leer por fichero
#include <fstream>
#include <vector>
...
int voraz(int L, const vi& v) {
    int ret = 0; // variable de retorno
    int i = 1, n = v.size(); // iteradores del vector
...
bool resuelveCaso() {
    ...
    cin >> N >> L;
    if (!cin) return false; // no hay mas datos por leer, se acaba
    vi v;
    int a;
    for (int i = 0; i < N; i++) {
        cin >> a;
